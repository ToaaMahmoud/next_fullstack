...
export default function ProductCard({ product, index = 0, cat = null }) {
  ...
  return (
    <div
      className={`group relative bg-paper border-thicker border-ink p-4 shadow-block-sm transition-transform hover:-translate-y-1 hover:shadow-block ${rot}`}
    >
      ...
      {cat && (
        <div
          className={`absolute -bottom-3 left-4 z-10 border-thick px-3 py-1 font-mono-tag -rotate-2 ${tagColor}`}
        >
          {/* {product.category} */}
          {cat}
        </div>
      ...
      <div className="pt-5 pb-1">
        ...
        <div className="mt-3 flex items-center gap-2">
          <button
            type="button"
            onClick={handleAdd}
            className={`flex-1 bg-ink text-paper px-3 py-2 font-mono-tag hover:bg-pop-red transition-colors ${added ? "opacity-60 pointer-events-none" : ""}`}
            disabled={added}
          >
            {added ? "Added!" : "Add +"}
          </button>
